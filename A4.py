...
def main():
    ...
    menu = st.sidebar.radio("Select Menu", ["Profile", "Graph", "City Connection Map"])
    ...
    if menu == "Profile":
    ...
    elif menu == "City Connection Map":
        st.title("City Connection Visualization")
        ...
        if not selected_cities:
            st.warning("Select at least one city to be show.")
            return

        if province_connections:
            st.write(f"Select City Connection for {selected_province}...")
            m = create_map(province_connections, city_coordinates, selected_cities)
            st_folium(m)
        else:
            st.error("Data Connection for this province is not found")
